...
//from，当前角度（弧度），也就是 _skin.Rotation.Y 的值
//to，目标角度（弧度），也就是 targetAngle
//step，本帧最多允许旋转的角度（弧度），一般是 delta * 速度
	{
		float diff = Mathf.AngleDifference(from, to);
		// 计算从 from 到 to 的最短角度差，比如从 350° → 10°，差值是 +20°（不是 -340°）
		if (Mathf.Abs(diff) <= step)
			return to;
		// 如果 当前角度与目标角度的差距 ≤ 本帧最大旋转量→ 直接把角度设置成目标角度
		//
...
//	•	diff < 0 → -1 → 逆最短方向转
//	•	from + Sign(diff) * step：
//	•	从当前角度出发，沿最短路径，旋转 step 弧度
// “朝正确的方向，用固定的小速度转”

//💡 核心思路：“每帧最多旋转 step，直到到目标”
//Mathf.Sign(x) 用来判断一个数是正的、负的，还是 0。它不关心大小，只关心方向（符号）。
	...
	private void HandleAttack()
	{
		...
		if (Input.IsActionJustPressed("attack"))
		{
			...
			//“在 AnimationNodeOneShot 这个类里的 OneShotRequest 枚举里，取值 Fire”
			_attacking = true;
			//•	作用：控制角色所有动画的“总控制台”
	        //•	比 AnimationPlayer 更高级（可以混合、状态机、OneShot）
			//这能防止玩家在攻击动画还没结束时，通过狂按键盘连续触发攻击，起到“防抽风”的作用。
			_attackTimer = AttackMaxDuration;
			//这是预设的攻击持续时间（例如 0.5 秒）
			//作用：既然设置了 _attacking = true，就需要一个机制把它改回 false。
			//这行代码初始化了一个倒计时，通常在 _Process 逻辑中会随时间递减。
			//当计时器归零时，角色才会恢复到“非攻击状态”。
